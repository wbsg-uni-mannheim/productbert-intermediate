...
class MnistModel(BaseModel):
    ...
    def forward(self, seq, token_ids, attn_masks):
        ...
        cont_reps, _ = self.bert_layer(seq, attention_mask=attn_masks, token_type_ids=token_ids)
    ...
    def __init__(self, num_classes_multi, num_classes=1, freeze_bert=False):
        ...
        # Classification layer
        self.binary_cls_layer = nn.Linear(768, num_classes)
        ...
        self._init_weights_bert(self.binary_cls_layer)
    ...
    def forward(self, seq, token_ids, attn_masks):
        ...
        # Feeding cls_rep to the classifier layer
        logits_binary = self.binary_cls_layer(cls_rep)
        logits_multi1 = self.multi1_cls_layer(cls_rep)
        logits_multi2 = self.multi2_cls_layer(cls_rep)
    ...
    def __init__(self, num_classes=1, freeze_bert=False, adapter_latent_size=128, layers_to_adapt=12, adapter_non_linearity="gelu"):
        super().__init__()
        layers_to_adapt = list(range(layers_to_adapt))
        c = RunConfigAdapted(adapter_latent_size, layers_to_adapt, adapter_non_linearity)
        cfg = BottleneckAdapterBertConfig.from_pretrained(c.model_name_or_path, **c.__dict__)
        self.bert_layer = AdapterBertModel.from_pretrained(c.model_name_or_path, config=cfg)
...
class BertModelSupplementaryPoolerMLM(BaseModel):
    ...
    def __init__(self, num_classes=1, freeze_bert=False, adapter_latent_size=128, layers_to_adapt=12, adapter_non_linearity="gelu"):
        ...
        # computers_new
        self.load_state_dict(torch.load('saved/models/BERT-computers-pretrain-adapter/0606_102008/model_with_head.bin'))
...
class BertModelSupplementaryMLM(BaseModel):
    ...
    def forward(self, seq, token_ids, attn_masks):
        ...
        # Feeding cls_rep to the classifier layer
        logits_binary = self.cls(pooler_output)
        logits_multi1 = self.multi1_cls_layer(pooler_output)
        logits_multi2 = self.multi2_cls_layer(pooler_output)
